...
try {
    $pdo = new PDO(
        'mysql:host=forel.mysql.ukraine.com.ua;dbname=forel_admchicago',
        'forel_admchicago',
        'xet5lrdl',
    ...
    $where = [];
    $conditions = [];
    $cheNaEmpt = $DB->query("SELECT COUNT(1) FROM `apartments`;");
    $FinEmptCheck = mysqli_fetch_array($cheNaEmpt);
    if($_GET['filter']=='all')
    {
        $where[] = "1";
